...
public class KitBuilder implements Listener {
     ...
     */
    public void init(){
        Agathe.getCache().getIPlayer(player).setState(PlayerState.BUILDING_KIT);
        Agathe.getCache().getIPlayer(player).setKitBuilder(this);
        Bukkit.getServer().getPluginManager().registerEvents(this, Agathe.getPlugin());
        player.teleport(Agathe.getIConfig().getKitBuilderSpawn());
        player.setHealth(20);
        player.setFoodLevel(20);
        Agathe.getCache().getIPlayer(player).equipKit(ladder);
        for(Player pl : Bukkit.getOnlinePlayers()){
            player.hidePlayer(pl);
        }
        active = true;
        Agathe.getCache().getIPlayer(player).getScoreboard().update();
    }
    ...
    public void onInteract(PlayerInteractEvent e){
        ...
        IPlayer iPlayer = Agathe.getCache().getIPlayer(p);
