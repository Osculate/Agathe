...
public class MatchHandler implements Listener {
    ...
    public void onDeath(PlayerDeathEvent e){
        ...
        if (match.getPlayerManager().hasPlayer(p)) {
            ...
            if (killer != null) {
                IPlayer kip = Agathe.getCache().getIPlayer(killer);
    ...
    public void onQuit(PlayerQuitEvent e) {
        ...
        if (match.getPlayerManager().hasPlayer(p)) {
            ...
            if (p.getKiller() != null) {
                IPlayer kip = Agathe.getCache().getIPlayer(p.getKiller());
                match.eliminatePlayer(ip, kip);
            } else {
                match.eliminatePlayer(ip, null);
            }
            ...
            if (match.getPlayerManager().hasPlayer(d)) {
                if (match.getPlayerManager().hasPlayer(p)) {
                    if (!match.isStarted()) {
                        ...
                        if (match.getTeamManager().getTeam(Agathe.getCache().getIPlayer(p)).getName()
                                .equalsIgnoreCase(match.getTeamManager().getTeam(Agathe.getCache().getIPlayer(d)).getName())) {
                            e.setCancelled(true);
                            e.setDamage(0.0);
